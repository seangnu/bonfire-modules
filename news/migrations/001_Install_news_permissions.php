...
class Migration_Install_news_permissions extends Migration {
    ...
    public function up() 
    {
        ...
        // permissions
        $this->db->query("INSERT INTO {$prefix}permissions VALUES (0,'News.Content.Manage','','active');");
    ...
    public function down() 
    {
        ...
        // permissions
        $query = $this->db->query("SELECT permission_id FROM {$prefix}permissions WHERE name='News.Content.Manage';");
        foreach ($query->result_array() as $row)
        {
                $permission_id = $row['permission_id'];
                $this->db->query("DELETE FROM {$prefix}role_permissions WHERE permission_id='$permission_id';");
        }
        $this->db->query("DELETE FROM {$prefix}permissions WHERE name='News.Content.Manage';");
